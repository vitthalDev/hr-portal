...
const CustomMenu = props => {
  ...
  const [activeProviders, setActiveProviders] = useState(false)
  const [aboutProviders, setAboutProviders] = useState(false)
    ...
    <Navbar>
      <Fragment>
        <NavItem>
          ...
          <NavLink>
            <div
              style={{ paddingTop: 15, paddingBottom: 15, paddingLeft: aboutProviders ? 50 : 15, width: 200, borderRadius: 15 }}
              onClick={(e) => {
                setActiveDashboard(false)
                setActivePolicies(false)
                setActiveEndorsement(false)
              ...
              onMouseOver={(e) => {
                ...
                setAboutEmployees(false)
                setAboutChallenges(false)
                setAboutProviders(true)
                setAboutClaims(false)
                setAboutBills(false)
                setAboutAnalytics(false)
              }}
              className={activeProviders ? 'text-center colors-container bg-gradient-primary rounded text-black  d-flex align-items-center justify-content-center shadow' : 'text-center colors-container bg-gradient-light rounded text-black  d-flex align-items-center justify-content-center '}
            >
              <Link to='/dashboard/providers' style={{ color: activeProviders ? 'white' : 'black' }}>
                <Icon.Package size={20} style={{ marginRight: 15 }} />
                <span style={{ marginInlineEnd: 40 }}>Providers</span>
              </Link>
            </div>
          </NavLink>
